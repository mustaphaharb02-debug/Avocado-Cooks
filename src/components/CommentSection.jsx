...
export default function CommentSection({ recipeId }) {
  ...
  const storageKey = `comments_${recipeId}`
  const saved = () => {
    try { return JSON.parse(localStorage.getItem(storageKey)) || [] }
    catch { return [] }
  }

  const [comments, setComments] = useState(saved)
    ...
    <section className="comment-section" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <form className="comment-form" onSubmit={handleSubmit} noValidate>
        ...
        <div className={`comment-form__field ${errors.text ? 'has-error' : ''}`}>
          <label className="comment-form__label">{t.yourComment}</label>
          <textarea
        ...
        <div className="comment-form__actions">
          ...
          {submitted && (
            <span className="comment-form__success animate-fadeIn">✓ {isRTL ? 'تم الإرسال!' : 'Submitted!'}</span>
          )}
        </div>
      </form>
      ...
      <div className="comment-list">
        {comments.length === 0 ? (
          <p className="comment-list__empty">{t.noComments} 🥑</p>
        ) : (
          comments.map(c => (
            <div key={c.id} className="comment-item animate-fadeInUp">
              ...
              <div className="comment-item__body">
                <div className="comment-item__header">
                  <strong className="comment-item__name">{c.name}</strong>
